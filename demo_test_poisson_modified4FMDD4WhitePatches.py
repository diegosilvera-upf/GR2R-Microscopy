...
from saturation_experiment import add_saturation_patches  # o donde la tengas definida

# ---------------------------------------------------------------
# Setup paths
# ---------------------------------------------------------------
BASE_DIR = Path(".")
PROJECT_NAME = "denoising-poisson-fmdd-drunet-white-patches"
...
def save_parameters(args, output_dir):
    ...
    with open(output_dir / "parameters.txt", "w") as f:
        f.write(f"Experiment: demo_test_poisson_modified4FMDD4WhitePatches.py\n")
...
def build_eval_cache(test_dataset, physics, n_eval, device, seed):
    ...
    with torch.no_grad():
        for i in range(n_eval):
            ...
            y_noisy = torch.clamp(y_noisy, max=1.0)  # Para estudiar efecto de saturación
...
def train_model(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x_clean_stack, x_clean in pbar:
            ...
            x_clean_img = x_clean_stack[:, 0:1, :, :].clone()
            optimizer.zero_grad()
            
            # Aplicar patches a cada imagen del batch con probabilidad SATURATION_PROB
            SATURATION_PROB = 0.5  # podés ponerlo como argumento en Args
            for b in range(x_clean_img.shape[0]):
                if torch.rand(1).item() < SATURATION_PROB:
                    img_np = x_clean_img[b, 0].cpu().numpy() * 255.0  # a escala cruda
                    patched = add_saturation_patches(img_np, seed=None)
                    x_clean_img[b, 0] = torch.tensor(patched / 255.0, device=device)

            # Aplicar ruido Poisson via physics: y ~ Poisson(x_clean * gamma) / gamma
            y = physics(x_clean_img)
            y = torch.clamp(y, max=1.0)  # saturación: clip en 255 equivale a clip en 1.0
