...
def load_weights(model: torch.nn.Module, weights_path: Path) -> None:
    ...
    # Si no hay overlap, intentar agregar "model." para checkpoint sin wrapper
    model_keys = set(model.state_dict().keys())
    if not (model_keys & set(cleaned.keys())):
        prefixed = {f"model.model.{k}": v for k, v in cleaned.items()}
...
# def load_weights(model: torch.nn.Module, weights_path: Path) -> None:
#     checkpoint = torch.load(weights_path, map_location=device, weights_only=False)
#     state_dict = checkpoint.get("state_dict", checkpoint) #For my weights
#     # state_dict = checkpoint.get("state_dict", checkpoint.get("model_state_dict", checkpoint)) #For Hortensia's weights
#     cleaned = {k: v for k, v in state_dict.items() if not k.startswith("noise_model.")}
#     missing, unexpected = model.load_state_dict(cleaned, strict=False)
#     if missing:
#         print(f"WARNING: {len(missing)} missing keys when loading weights")
