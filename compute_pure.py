...
def pure_loreal_sequence(
   ...
   if model_type == "drunet":
       ...
       for i in tqdm(indices, desc=seq_name, leave=False):
           ...
           samples = [
               compute_pure_mc(model, y_central, physics, noise_level, y_stack, plain=plain)
               for _ in range(n_b)
           ]
           frame_pures.append(float(np.mean(samples)))
...
def main() -> None:
   ...
   parser.add_argument(
       ...
       help="Bypass R2RModel's stochastic test-time ensembling (which otherwise averages "
            "eval_n_samples random recorruptions on every forward call) for both the true-MSE "
            "reference and the PURE estimator's forward passes. Single deterministic pass "
            "through the trained backbone.",
   )
   ...
   if args.dataset == "loreal":
       ...
       for seq in fmdd_seqs:
           ...
           if args.fmdd_mode == "clean":
               row = pure_fmdd_clean(model, args.model, physics, seq, noise_level, args.n_b, args.plain)
           else:
               row = pure_fmdd_raw(
