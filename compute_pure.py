...
def pure_fmdd_clean(
    ...
    frame_pures = [float(np.mean(samples))]
    return {
        "sequence": f"{seq['modality']}/{seq['seq_id']}",
        "n_frames": 1,
        "pure_mean": frame_pures[0],
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory with GT TIF files named <sequence_name>.tif (one per sequence). "
    ...
    parser.add_argument(
        "--per-frame",
        action="store_true",
        help="Write one CSV row per frame instead of one summary row per sequence",
    )
    parser.add_argument(
    ...
    if args.dataset == "loreal":
        ...
        for tif_files in tif_seqs:
            seq_name = tif_files[0].parent.name
            gt_path = (args.gt_dir / f"{seq_name}.tif") if args.gt_dir is not None else None
    ...
    if args.per_frame:
        # Expand: one row per frame
        out_rows: list[dict] = []
        for r in rows:
            seq = r["sequence"]
            for fi, pure_val in enumerate(r["frame_pures"]):
                entry: dict = {"sequence": seq, "frame": fi, "pure": pure_val}
                if "frame_mses" in r:
                    entry["true_mse"] = r["frame_mses"][fi] if fi < len(r["frame_mses"]) else ""
                out_rows.append(entry)
        fieldnames = list(out_rows[0].keys()) if out_rows else ["sequence", "frame", "pure"]
    else:
        out_rows = [{k: v for k, v in r.items() if k not in ("frame_pures", "frame_mses")} for r in rows]
        fieldnames = []
        for r in out_rows:
            for k in r:
